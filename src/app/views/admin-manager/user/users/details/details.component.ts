...
export class DetailsComponent {
 ...
 ngOnInit() {
  ...
  this.selectedUser = {
    ...
    profilePicture: this.userForm.value.profilePicture || new Uint8Array(),
      ...
      if (roles) {
        ...
        this.userForm.value.roles.forEach((role: { roleName: string; id: number; }, index: number) => {
          ...
          if (existingRole) {
            ...
            this.editUserForm.get(`roles.${index}.id`)?.patchValue(existingRole.id); // Utilisation de la vérification de nullité (?)
          }
        });
      }
    },
